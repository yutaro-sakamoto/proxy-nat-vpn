...
describe('ProxyNatVpn', () => {
  test('VPC is created with public and private subnets', () => {
    ...
    new ProxyNatVpn(stack, 'MyProxyNatVpn', {
      clientVpnServerCertificateArn: 'arn:aws:acm:region:account:certificate/server-cert-id',
      clientVpnClientCertificateArn: 'arn:aws:acm:region:account:certificate/client-cert-id',
    });
  ...
  test('Client VPN Endpoint is created with correct properties', () => {
    ...
    template.hasResourceProperties('AWS::EC2::ClientVpnEndpoint', {
      ServerCertificateArn: 'arn:aws:acm:region:account:certificate/server-cert-id',
      AuthenticationOptions: [
        {
          MutualAuthentication: {
            ClientRootCertificateChainArn: 'arn:aws:acm:region:account:certificate/client-cert-id',
          },
